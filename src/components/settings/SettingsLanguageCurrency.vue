    ...
    <div class="glass-panel rounded-2xl p-6">
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          >
            <option value="fr" class="bg-zinc-900">🇫🇷 {{ $t('language.fr') }}</option>
            <option value="en" class="bg-zinc-900">🇺🇸 {{ $t('language.en') }}</option>
          </select>
          <p class="text-xs text-zinc-400 mt-2">{{ $t('settings.languageDescription') }}</p>
        </div>
        ...
        <div>
          ...
          <select
            ...
            class="w-full max-w-xs bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2 focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 outline-none transition-colors"
          >
            <option value="EUR" class="bg-zinc-900">{{ $t('currency.EUR') }}</option>
            <option value="USD" class="bg-zinc-900">{{ $t('currency.USD') }}</option>
            <option value="GBP" class="bg-zinc-900">{{ $t('currency.GBP') }}</option>
            <option value="XOF" class="bg-zinc-900">{{ $t('currency.XOF') }}</option>
          </select>
          <p class="text-xs text-zinc-400 mt-2">{{ $t('settings.currencyDescription') }}</p>
        </div>

        <!-- Aperçu en direct -->
        <div class="mt-8 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
          <p class="text-sm font-medium text-white mb-2">{{ $t('settings.preview') }}</p>
          <p class="text-sm text-zinc-300">
            {{ $t('settings.previewExample') }}:
            <span class="font-semibold text-emerald-400">{{ formatCurrency(1200) }}</span>
          </p>
        </div>
      </div>
    </div>
