<template>
  <template>
    ...
    >
      ...
      <div
        class="absolute -right-20 -top-20 w-40 h-40 bg-violet-500/10 rounded-full blur-3xl group-hover:bg-violet-500/20 transition-all duration-500 opacity-0 group-hover:opacity-100"
      ></div>
      ...
      <div class="flex-1 flex flex-col p-6 relative z-10">
        ...
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1 min-w-0 pr-4">
            <h3
              class="text-lg font-bold text-white mb-1 truncate group-hover:text-violet-300 transition-colors"
            >
              {{ property.name }}
            </h3>
            <p class="text-sm text-zinc-400 mb-1 truncate">{{ property.city }}</p>
            <p
              v-if="property.surface || property.type"
              class="text-xs text-violet-300 mb-1 font-medium"
            >
