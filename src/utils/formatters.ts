 ...
 */
let settingsStoreCache: any = null
...
function getSettingsStore() {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && (window as any).__doogoo_settingsStore) {
      settingsStoreCache = (window as any).__doogoo_settingsStore
      return settingsStoreCache
    }
  } catch {
    // Ignore les erreurs
 ...
 * @param store - Le store settings
 */
export function setSettingsStoreCache(store: any) {
  settingsStoreCache = store
  if (typeof window !== 'undefined') {
    ;(window as any).__doogoo_settingsStore = store
  }
}
 ...
 */
export function formatCurrency(amount: number | null | undefined, options: any = {}): string {
 ...
 * @param dateString - Date à formater (ISO string ou Date)
 * @param options - Options de formatage
