 ...
 * @param error - L'erreur retournée par Supabase ou une erreur JavaScript
 * @param context - Contexte de l'erreur (ex: "fetchProperties", "createPayment")
...
export function handleApiError(error: any, context: string = '') {
  ...
  try {
    ...
    if (errorStore.isNetworkOrTimeoutError(error)) {
      ...
      const userMessage =
        message.includes('timeout') || message.includes("l'opération a pris plus de")
 ...
 * @param apiCall - Fonction async qui retourne { data, error }
 * @param context - Contexte pour les logs
 * @returns {Promise<any>} { success: boolean, data?: any, error?: Error, retries?: number }
 */
  ...
  const wrappedApiCall = async () => {
    try {
      ...
      if (
        ...
        error?.message?.includes("l'opération a pris plus de")
      ) {
        try {
          const errorStore = useErrorStore()
          errorStore.triggerNetworkError(
