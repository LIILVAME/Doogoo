 ...
 * @param {string} userId - ID de l'utilisateur (pour la sécurité via RLS)
 * @returns {Promise<Object>} { success: boolean, data?: Object, error?: Error }
 */
export async function uploadDocument(tenantId, file, userId) {
 ...
 */
export async function listDocuments(tenantId, userId) {
  if (!tenantId || !userId) {
 ...
 * @returns {Promise<Object>} { success: boolean, data?: { signedUrl: string }, error?: Error }
 */
export async function getDocumentUrl(tenantId, fileName, userId, expiresIn = 3600) {
 ...
 */
export async function deleteDocument(tenantId, fileName, userId) {
  if (!tenantId || !fileName || !userId) {
    return { success: false, message: 'Tenant ID, file name et User ID requis' }
  }
