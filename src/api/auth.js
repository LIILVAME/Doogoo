 ...
 * @returns {Promise<Object>} { success: boolean, data?: string (URL), message?: string }
 */
export async function uploadPropertyImage(propertyId, file, userId) {
  if (!userId) {
    return { success: false, message: 'User ID requis' }
  }
  ...
  return withErrorHandling(
    async () => {
      const fileExt = file.name.split('.').pop()
      const fileName = `properties/${userId}/${Date.now()}.${fileExt}`
      const filePath = fileName
 ...
 * @returns {Promise<Object>} { success: boolean, data?: string (URL), message?: string }
 */
export async function uploadSignature(userId, file) {
  if (!userId) {
    return { success: false, message: 'User ID requis' }
  }
 ...
 */
export async function checkUserHasProperties(userId) {
  if (!userId) {
    return { success: false, message: 'User ID requis' }
  }
 ...
 */
export async function setSessionFromResetToken(accessToken, refreshToken) {
