...
export interface PropertyParams {
  ...
  status?: string
  user_id?: string
  zip?: string | number // Fix type to allow number or string as consistent with usage
  description?: string
  type?: string
 ...
 */
export async function getProperties(userId: string): Promise<ApiResponse> {
  if (!userId) {
 ...
 */
export async function getPropertyById(propertyId: string, userId: string): Promise<ApiResponse> {
 ...
 */
export async function deleteProperty(propertyId: string, userId: string): Promise<ApiResponse> {
  if (!propertyId || !userId) {
    return { success: false, message: 'Property ID et User ID requis' }
  }
