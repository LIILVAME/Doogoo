...
export const useAnalyticsStore = defineStore('analytics', () => {
  ...
  // Données brutes
  const revenueByMonth = ref<{ month: string; total: number }[]>([])
  const revenueByProperty = ref<{ name: string; total: number; id?: string }[]>([])
  ...
  const previousPeriod = ref<any>(null)
  const periodComparison = ref<any>(null)

  /**
   * Récupère toutes les données analytiques depuis Supabase
   * @param options - Options de période { currentStart, currentEnd, previousStart, previousEnd }
   */
  const fetchAnalytics = async (options: any = {}) => {
    loading.value = true
    error.value = null
    ...
    try {
      ...
      const last12Months: { month: string; monthNum: number; year: number; total: number }[] = []
      ...
      const revenueByPropertyMap = new Map<string, { name: string; total: number }>()
      paidPayments.forEach((payment: any) => {
      ...
      const occupiedProperties = properties?.filter((p: any) => p.status === 'occupied').length || 0
      occupancyRate.value =
        totalProperties > 0 ? Math.round((occupiedProperties / totalProperties) * 100) : 0

      // 6️⃣ Compte les retards de paiement
      latePayments.value = payments?.filter((p: any) => p.status === 'late').length || 0

      // 7️⃣ Revenu total (tous les paiements payés)
      totalRevenue.value = paidPayments.reduce(
        (sum: number, p: any) => sum + Number(p.amount || 0),
        0
      )

      // 8️⃣ Loyer moyen
      const propertiesWithRent = properties?.filter((p: any) => p.rent && p.rent > 0) || []
      if (propertiesWithRent.length > 0) {
        averageRent.value =
          propertiesWithRent.reduce((sum: number, p: any) => sum + Number(p.rent), 0) /
          propertiesWithRent.length
      }
      ...
      paymentStatusBreakdown.value = {
        ...
        late: payments?.filter((p: any) => p.status === 'late').length || 0
      }
   ...
   */
  const fetchPeriodComparison = async (payments: any[], properties: any[], options: any) => {
